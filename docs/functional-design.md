...
│  │                                                          │   │
│  │  ② PostToolUse フック（全ツール完了後に発火）             │   │
│  │    └─ boo_notify.sh working                              │   │
│  │         └─ POST http://host.docker.internal:8765/working │   │
│  │                                                          │   │
│  │  ③ Stop フック（Claude の応答完了後に発火）               │   │
│  │    └─ boo_notify.sh idle                                 │   │
...
│  │                                                         │    │
│  │  /sse      ← MCP SSE エンドポイント                     │    │
│  │  /request  ← REST 承認エンドポイント（PreToolUse フック）│    │
│  │  /working  ← 作業中通知（PostToolUse フック）            │    │
│  │  /idle     ← アイドル通知（Stop フック）                 │    │
│  │                                                         │    │
│  │  BooDevice                                              │    │
...
### 7-4. PostToolUse/Stop フックによる working/idle 通知

```
Claude Code      boo_notify.sh    boo_bridge.py    M5StickC PLUS2
    │                  │                │                │
    │ ツール実行完了    │                │                │
    │ PostToolUse 発火  │                │                │
    │ stdin: tool payload               │                │
...
**フェイルオープン:** boo_bridge.py 未起動時は curl がタイムアウト（3秒）後に終了し、Claude Code の動作には影響しない。
