...
> ⚠️ **ボードパッケージとライブラリは別物。** ボードパッケージをインストールしても
> `M5StickCPlus2.h` は自動では入らない。次の C でライブラリも必ず確認すること。

---

### C. M5StickCPlus2 ライブラリの確認

`M5StickCPlus2.h` は **M5StickCPlus2 ライブラリ** として別途インストールが必要。

1. **Tools > Manage Libraries...** を開く（または `Ctrl + Shift + I`）
2. 検索欄に `M5StickCPlus2` と入力する
3. **M5Stack** 製の **M5StickCPlus2** が **INSTALLED** になっていることを確認する

インストールされていない場合:
1. **M5StickCPlus2** の **Install** ボタンをクリックする
2. 依存ライブラリのインストールを尋ねるダイアログが表示されたら **Install all** を選択する
3. インストール完了まで待つ

---

### D. USB 接続と COM ポートの確認
...
### E. スケッチファイルの場所を確認する（Dev Container 環境）

この環境は **VS Code Dev Container** で動作しているため、スケッチファイルは
Windows の C: ドライブ上に存在する。コンテナ内のパス（`/workspaces/...`）は
Windows から直接アクセスできない。

**Windows 側の実際のパスを確認する手順:**

1. VS Code の左側 **エクスプローラー** パネルで `src/boo_device.ino` を右クリックする
2. **Reveal in Explorer（エクスプローラーで表示）** を選択する
3. Windows エクスプローラーが開くので、アドレスバーに表示されているパスをメモする
   （例: `C:\Users\username\Documents\claude-pocket-approver-2\src`）

> **「Reveal in Explorer」が表示されない場合:**  
> VS Code のコマンドパレット（`Ctrl + Shift + P`）で
> `Reveal in Explorer` と入力して実行する。

Arduino IDE でスケッチを開く際は、上記で確認した Windows パスを使用する。
...
事前確認 E で確認した Windows パスを使用する。

1. VS Code の左側 **エクスプローラー** パネルで `src/boo_device.ino` を右クリックする
2. **Reveal in Explorer（エクスプローラーで表示）** を選択する
3. Windows エクスプローラーが開いたら、アドレスバーのパスをコピーする
4. Arduino IDE のメニューバーから **File > Open** を選択する
5. 開いたダイアログのアドレスバーにコピーしたパスを貼り付けて Enter を押す
6. `boo_device.ino` を選択して **開く** をクリックする
7. スケッチがエディタに表示されることを確認する
...
>   → M5StickCPlus2 **ライブラリ**が未インストール（事前確認 C を実施）
>     ボードパッケージ（B）とライブラリ（C）は別々のインストールが必要
