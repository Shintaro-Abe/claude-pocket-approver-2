...
- [x] **T-01** Arduino IDE で Partition Scheme を変更する
...
- [x] **T-02** ファイルヘッダーのバージョン文字列と通信プロトコル説明を更新する
  - `v2.0` → `v2.1`
  - `JSON over USB Serial (115200bps)` → `JSON over Bluetooth Classic SPP`
  - `USB Serial (115200bps) はデバッグ出力専用` を追記

- [x] **T-03** `#include <BluetoothSerial.h>` を追加する（`<ArduinoJson.h>` の直後）

- [x] **T-04** BT 用定数を定数セクションに追加する
  ```cpp
  // ---- Bluetooth ----
...
- [x] **T-05** `BluetoothSerial SerialBT;` をグローバル変数の先頭に宣言する
  - 配置: `DeviceState` 列挙型の直前

- [x] **T-06** `DeviceState` 列挙型に `ST_BT_WAIT` を追加する
  ```cpp
  enum DeviceState {
...
- [x] **T-07** `ART_BT_WAIT` 定数を追加する（`ART_TIRED` の直後）
  ```cpp
...
- [x] **T-08** `drawBtWaitScreen()` 関数を新設する（`drawStatsScreen()` の直後）
...
- [x] **T-09** `checkBtConnection()` 関数を新設する（`drawBtWaitScreen()` の直後）
...
- [x] **T-18** 起動直後に `ST_BT_WAIT` 画面（`"waiting BT..."` 点滅）が表示されることを確認する（AC-CONN-04）

- [x] **T-19** Windows 11 の Bluetooth 設定でデバイス名 `BooDevice`、PIN `1234` でペアリングできることを確認する（AC-CONN-01/03）

- [x] **T-20** デバイスマネージャーで仮想 COM ポートが割り当てられていることを確認する（AC-CONN-02）

- [x] **T-21** `boo_bridge.py --port /dev/ttyS<N>` を実行し `[boo] Connected` が表示されることを確認する（AC-CONN-02）

- [x] **T-22** 接続後にデバイスが `ST_IDLE` 画面（BT インジケーター `BT` 表示）に遷移することを確認する（AC-CONN-05/07）

### 切断・再接続テスト

- [x] **T-23** `boo_bridge.py` を Ctrl+C で終了し、5 秒以内に `ST_BT_WAIT` 画面へ遷移することを確認する（AC-CONN-06）

- [x] **T-24** `boo_bridge.py` を再起動し、再ペアリングなしで `ST_IDLE` へ戻ることを確認する（AC-CONN-07）

### 通信テスト

- [x] **T-25** `approve_request` ツールを呼び出し、承認画面が表示されることを確認する（AC-COMM-01）

- [x] **T-26** ボタン A で `{"approved":true}`、ボタン B で `{"approved":false}` が返ることを確認する（AC-COMM-02/03）

- [x] **T-27** Arduino Serial Monitor（115200bps）でデバッグログが表示され、JSON 文字列が混入しないことを確認する（AC-COMM-04/05）

### 点滅・アニメーションテスト

- [x] **T-28** `ST_BT_WAIT` 画面で `"waiting BT..."` が 600ms 間隔で点滅することを確認する（AC-CONN-08）

### 状態遷移テスト

- [x] **T-29** `ST_APPROVAL` 中に BT 切断（`boo_bridge.py` 強制終了）→ `ST_BT_WAIT` 画面に遷移し、
  再起動後に `ST_IDLE` へ戻ることを確認する（AC-TAMA-01）

- [x] **T-30** T-29 の遷移前後で `gBoo.fed` / `gBoo.energy` のゲージ値が保持されていることを目視確認する（AC-TAMA-02）
